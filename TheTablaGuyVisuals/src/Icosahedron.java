...
class Icosahedron {
  ...
  Icosahedron(PApplet _p, int level) {
    float sqrt5 = PApplet.sqrt(5);
    float phi = (1 + sqrt5) * 0.5f;
    float ratio = PApplet.sqrt(10 + (2 * sqrt5)) / (4 * phi);
  ...
  void makeIcosphereFace(PVector a, PVector b, PVector c, int level) {
    ...
    if (level <= 1) {
      ...
      PVector ta = new PVector(PApplet.atan2(a.z, a.x) / PConstants.TWO_PI + 0.5f, PApplet.acos(a.y) / PConstants.PI);
      PVector tb = new PVector(PApplet.atan2(b.z, b.x) / PConstants.TWO_PI + 0.5f, PApplet.acos(b.y) / PConstants.PI);
      PVector tc = new PVector(PApplet.atan2(c.z, c.x) / PConstants.TWO_PI + 0.5f, PApplet.acos(c.y) / PConstants.PI);
